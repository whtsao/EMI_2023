...
# translate caisson to middle of the tank
caisson2.translate(np.array([0.5*tank_length, water_level+fb_ly-yst+spacing+0.5*tld_ly]))
...
body.setInertiaXX(np.array([1., 1., ib]))
# record values
body.setRecordValues(all_values=True)

# SPRING 1 (left tilted)
TSDA1 = pychrono.ChLinkTSDA()
body1_point = pychrono.ChVectorD(0.5*tank_length-0.5*fb_lx,water_level+fb_ly-yst,0.0)
body2_point = pychrono.ChVectorD(0.5*tank_length+0.5*tld_lx,water_level+fb_ly-yst+spacing,0.0)
...
#body1_point = pychrono.ChVectorD(0.5*tank_length,water_level+fb_ly-yst,0.0)
#body2_point = pychrono.ChVectorD(0.5*tank_length,water_level+fb_ly-yst+spacing,0.0)
